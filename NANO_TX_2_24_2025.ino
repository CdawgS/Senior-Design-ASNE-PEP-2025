...
// Data structure for transmission (Optimized for 32-byte limit)
...
void setup() {
    ...
    radio.setDataRate(RF24_250KBPS); // Reliable data rate
...
void loop() {
    ...
    // Read analog values (reduce resolution for faster transmission)
    ...
    // Map values to 8-bit (0-255) to fit within the 32-byte limit

    uint8_t rudderValue = map(rawRudder, 0, 1023, 0, 255);

    uint8_t throttleValue = map(rawThrottle, 0, 1023, 0, 255);
 
    // Determine throttle mode (speed limit)

    uint8_t throttleMode;

    if (rawThrottleLimiter < 204) {

        throttleMode = 0;  // 20% max throttle

    } else if (rawThrottleLimiter < 409) {

        throttleMode = 1;  // 40% max throttle

    } else if (rawThrottleLimiter < 614) {

        throttleMode = 2;  // 60% max throttle

    } else if (rawThrottleLimiter < 819) {

        throttleMode = 3;  // 80% max throttle

    } else {

        throttleMode = 4;  // 100% max throttle
